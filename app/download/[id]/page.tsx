...
function DesktopCertificate({
  ...
  return (
    <div className="cert-outer" ref={certRef}>
        ...
        <div className="bottom-row">
          ...
          <div className="qr-block">
            ...
            <span className="text-xs capitalize">
              Scan QR code for Verification link
            </span>
            <span className=" qr-caption">
              (Static QR code will not accept)
            </span>
          </div>
    ...
    <div className="mob-cert">
      ...
      <div className="mob-content">
        ...
        <p className="mob-body">
          ...
          <strong>Social Work &amp; Human Management</strong> at{" "}
          <strong>
            Opus Social Foundation, Nasnoda District Centre Jaipur
          </strong>
          , from <strong>{fmt(data.internshipStartDate)}</strong> to{" "}
        ...
        <div className="mob-bottom">
          ...
          <div className="mob-qr">
            ...
            <span className="text-xs">Scan QR code for Verification link</span>
            <span className="qr-caption">(Static QR code will not accept)</span>
          </div>
        </div>

        <div className="mob-divider" />

        {/* Meta grid — 4 new fields added */}
        <div className="mob-meta-grid">
          {[
            ...
            ["Timing", "8 AM to 12 PM"],
            ["Duration", "120 Hours"],
            ["Programme", "Social Work & Human Mgmt."],
            ["Centre", "Nasnoda District Jaipur"],
          ].map(([label, value], i) => (
            <div
              className={`mob-meta-entry${i === 0 ? " full-width" : ""}`}
              key={label}
            >
              <span className="meta-label">{label}</span>
              <span className="meta-value">{value}</span>
...
// ─── Download Button (frontend PDF via html2canvas + jsPDF) ───────────────────
...
function DownloadButton({
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      // html2canvas-pro is a drop-in replacement that supports modern CSS color
      // functions (lab, oklab, color-mix) which the original html2canvas crashes on.
      // Install with: npm install html2canvas-pro jspdf
...
export default function CertificateVerifyPage() {
  ...
  // Ref points to the desktop certificate card — used by html2canvas
  ...
  return (
    <>
      ...
      <div className="page-wrapper">
        ...
        {/* Desktop — certRef attached here for html2canvas capture */}
        <div className="show-desktop">
          <DesktopCertificate data={data} certRef={certRef} />
        </div>
        ...
        <DownloadButton certificateId={data.certificateId} certRef={certRef} />
...
.valid-badge {
  ...
  letter-spacing: 0.12em; color: #5dcaa5; text-transform: uppercase;
}
...
.id-badge {
  ...
  display: flex; flex-direction: column; align-items: flex-end; gap: 2px; z-index: 5;
}
.id-label {
  font-family: 'Cinzel', serif; font-size: 0.5em;
  letter-spacing: 0.15em; color: #c0a060; text-transform: uppercase;
}
.id-value {
  font-family: 'Cinzel', serif; font-size: 0.65em;
  letter-spacing: 0.1em; color: #9a7a30; text-transform: uppercase;
}

.org-header { display: flex; align-items: center; gap: 10px; margin-bottom: 0.5%; }
.org-dot { width: 5px; height: 5px; background: #c9a84c; border-radius: 50%; flex-shrink: 0; }
.org-name {
  font-family: 'Cinzel', serif; font-size: 0.9em; font-weight: 600;
  letter-spacing: 0.35em; color: #7a5c10; text-transform: uppercase; white-space: nowrap;
}

.ribbon-wrap {
  display: flex; align-items: center; width: 100%; margin: 0.5% 0 0.4%; gap: 14px;
}
.ribbon-line { flex: 1; height: 1px; background: linear-gradient(90deg, transparent, #c9a84c); }
.ribbon-line.flip { transform: scaleX(-1); }
.cert-title {
  font-family: 'Cinzel', serif; font-size: 2.6em; font-weight: 900;
  color: #5c3d02; letter-spacing: 0.08em; text-transform: uppercase;
  line-height: 1; text-shadow: 0 1px 2px rgba(100,60,0,0.18); white-space: nowrap;
}
.cert-subtitle {
  font-family: 'Cinzel', serif; font-size: 0.72em; letter-spacing: 0.4em;
  color: #9a7a30; text-transform: uppercase; margin: 0.3% 0 0.5%;
}
.star-row { color: #c9a84c; font-size: 0.65em; letter-spacing: 0.3em; margin: 0.3% 0 0.5%; }

.presented {
  font-family: 'Cormorant Garamond', serif; font-style: italic;
  font-size: 1em; color: #7a6040; letter-spacing: 0.05em; margin-bottom: 0.2%;
}
.recipient-name {
  font-family: 'Cinzel', serif; font-size: 2.1em; font-weight: 700;
  color: #3b2000; letter-spacing: 0.08em; margin: 0.3% 0 0.2%;
  text-shadow: 0 1px 0 rgba(200,160,50,0.3);
}
.father-line {
  font-family: 'Cormorant Garamond', serif; font-style: italic;
  font-size: 0.92em; color: #7a6040; margin-bottom: 0.5%;
}
.father-name { font-weight: 600; font-style: normal; color: #4a2e00; }

.body-text {
  font-family: 'Cormorant Garamond', serif; font-size: 0.92em;
  color: #3d2800; line-height: 1.6; text-align: center;
  max-width: 90%; margin: 0.6% auto 0;
}
...
.bottom-row {
  ...
  justify-content: space-between; padding-top: 0.6%;
}

/* Two-column meta block */
.meta-two-col {
  display: flex; gap: 16px; width: 42%;
}
.meta-block { flex: 1; }
.meta-entry {
...
.meta-label {
  ...
  letter-spacing: 0.13em; color: #9a7a30; text-transform: uppercase;
}
.meta-value {
  font-family: 'Cormorant Garamond', serif;
  font-size: 0.8em; color: #6b4c1e; line-height: 1.4;
}

.qr-block { display: flex; flex-direction: column; align-items: center; gap: 4px; }
.qr-border {
  padding: 5px; border: 1.5px solid #c9a84c;
  background: #fffdf5; box-shadow: 0 2px 12px rgba(100,70,0,0.12);
}
.qr-img { width: 72px; height: 72px; display: block; }
.qr-caption {
  font-family: 'Cinzel', serif; font-size: 0.5em;
  letter-spacing: 0.18em; color: #9a7a30; text-transform: uppercase;
}

.sig-block { width: 22%; display: flex; flex-direction: column; align-items: center; }
.sig-images {
  display: flex; flex-direction: row; align-items: flex-end;
  justify-content: center; gap: 8px; margin-bottom: 1px;
}
.sig-sign { height: 8em; max-width: 18em; object-fit: contain; }
.sig-seal { height: 10em; max-width: 18em; object-fit: contain; }
.sig-line {
  width: 85%; height: 1px;
...
.sig-label {
  font-family: 'Cinzel', serif; font-size: 0.5em;
  letter-spacing: 0.2em; color: #8b6914; text-transform: uppercase; text-align: center;
}
.sig-sublabel {
  font-family: 'Cormorant Garamond', serif; font-style: italic;
  font-size: 0.68em; color: #7a6040; text-align: center;
}
...
.mob-cert {
  ...
  box-shadow:
    0 0 0 8px #8b6914, 0 0 0 11px #f5ead2,
    0 0 0 13px #7a5c10, 0 0 0 15px #c9a84c,
...
.mob-id {
  ...
  display: flex; flex-direction: column; align-items: flex-end; gap: 1px; z-index: 5;
}
.mob-id-val {
  font-family: 'Cinzel', serif; font-size: 9px;
  letter-spacing: 0.08em; color: #9a7a30; text-transform: uppercase;
}
...
.mob-org {
  ...
  letter-spacing: 0.2em; color: #7a5c10; text-transform: uppercase; text-align: center;
}
.mob-divider {
...
.mob-title {
  ...
  color: #5c3d02; letter-spacing: 0.06em; text-transform: uppercase;
  text-align: center; line-height: 1.15; text-shadow: 0 1px 2px rgba(100,60,0,0.18);
}
.mob-subtitle {
  font-family: 'Cinzel', serif; font-size: 9px; letter-spacing: 0.3em;
  color: #9a7a30; text-transform: uppercase; text-align: center;
}
.mob-stars { color: #c9a84c; font-size: 11px; letter-spacing: 0.3em; }
.mob-presented {
  font-family: 'Cormorant Garamond', serif; font-style: italic;
  font-size: 14px; color: #7a6040; text-align: center;
}
.mob-name {
  font-family: 'Cinzel', serif; font-size: 24px; font-weight: 700;
  color: #3b2000; letter-spacing: 0.06em; text-align: center; line-height: 1.2;
  text-shadow: 0 1px 0 rgba(200,160,50,0.3);
}
.mob-father {
  font-family: 'Cormorant Garamond', serif; font-style: italic;
  font-size: 13px; color: #7a6040; text-align: center;
}
.mob-body {
  font-family: 'Cormorant Garamond', serif; font-size: 13.5px;
  color: #3d2800; line-height: 1.7; text-align: center;
}
...
.mob-sig { display: flex; flex-direction: column; align-items: center; flex: 1; }
...
.mob-sig-label {
  ...
  letter-spacing: 0.18em; color: #8b6914; text-transform: uppercase; text-align: center;
}
.mob-sig-sub {
  font-family: 'Cormorant Garamond', serif; font-style: italic; font-size: 10px; color: #7a6040;
}
.mob-qr { display: flex; flex-direction: column; align-items: center; gap: 4px; }
.mob-qr-img { width: 70px; height: 70px; display: block; }

/* Mobile meta grid */
.mob-meta-grid {
  width: 100%; display: grid; grid-template-columns: 1fr 1fr; gap: 6px 16px;
}
.mob-meta-entry {
  border-bottom: 0.5px solid rgba(180,140,80,0.3);
  padding-bottom: 3px; display: flex; flex-direction: column;
}
.mob-meta-entry.full-width { grid-column: 1 / -1; }
...
.download-btn {
  ...
  color: #f5d27a; background: transparent;
  border: 1px solid #c9a84c; border-radius: 4px;
  cursor: pointer; transition: background 0.2s; white-space: nowrap;
}
.download-btn:hover:not(:disabled) { background: rgba(201,168,76,0.12); }
...
.hint {
  font-family: 'Cormorant Garamond', serif; font-style: italic;
  font-size: 13px; color: rgba(200,170,110,0.55);
...
.shimmer-line {
  ...
  background: linear-gradient(90deg,
    rgba(200,160,60,0.08) 25%, rgba(200,160,60,0.18) 50%, rgba(200,160,60,0.08) 75%);
  background-size: 200% 100%; animation: shimmer 1.5s infinite;
}
.error-card {
  background: rgba(154,48,16,0.08); border: 1px solid rgba(154,48,16,0.25);
  border-radius: 12px; padding: 40px 48px;
  display: flex; flex-direction: column; align-items: center; text-align: center; max-width: 440px;
}
.error-title { font-family: 'Cinzel', serif; font-size: 18px; color: #5c2a10; margin-bottom: 8px; }
.error-msg   { font-family: 'Cormorant Garamond', serif; font-size: 15px; color: #7a5c40; }
`;
